...
| 2  |  the "Job offers" page does not show which job is the current job. |  Hui | Ricahrd  |  resolved | |
...
| 4. | in "job offers" page, the "cost of living index" blank can still be inputed. Is this design by purpose? I thought we want to calculate the cost of living index by the database.| Hui | Richard | canceled | Alex said it was fine|
...
| .7 | Allow users to update thier current job's info  | Richard | Richard | resolved | |
| .8 | Title is not being saved correctly from job offers | Richard | Richard | resolved | |
| .9 | compare job offers (disabled if no job offers were entered yet | Richard | Richard | resolved|To be precise, this functionality will be enabled if there are either (1) at least two job offers, in the case there is no current job, or (2) at least one job offer, in the case there is a current job. |
...
|20|Current job deleted when updated|Richard| Richard | resolved | |
|21|Edit Job offer does not update DB|Richard| Richard | open | |
| | | | | | |
